...
//            $('#myModal').modal('show');
	...
	jQuery('#drawChart').on('click', function(event) {    
		 var drawChartId=$('#tableSelectedProduct').val();
           if(drawChartId!=""){   
		     drawProductChart(drawChartId);
...
myChart.series[0].remove(true);
 ...
 function drawProductChart(prId){
 
     $.getJSON('http://localhost:8080/api/product/findByProductId?productId='+prId, function (data) {
		 var chartDataArr =[];
